...
class AutomatedNetworkTest:
    def __init__(self, net, verify_cleanup=True, test_count=100):
        self.net = net
    ...
    def run_ditg_sender(self, src_host, dst_host, port, test_id, duration):
        ...
        # D-ITG script for 3Mbps CBR traffic
        script_content = f"-a {dst_ip} -sp {port} -C 3000 -t {duration*1000} -l {output_file}"
        script_file = f"/tmp/ditg_script_{src_host}_{test_id}.txt"
        
        src_obj = self.net.get(src_host)
        src_obj.cmd(f"echo '{script_content}' > {script_file}")
        
        cmd = f"ITGSend {script_file}"
    ...
    def ditg_thread(self, config, test_id):
        """Thread 3: Manage D-ITG TCP traffic"""
        ...
        try:
            ...
            for pair_config in config:
                ...
                pair_config['ditg_port'] = port
            
            # Start senders
            for pair_config in config:
                tcp_src, tcp_dst = pair_config['tcp']
                port = pair_config['ditg_port']
            ...
            while not self.stop_event.is_set() and not self.error_event.is_set():
                ...
                for proc, host, _ in receivers + senders:
                    if proc.poll() is not None and time.time() - start_time < self.test_duration:
                        self.logger.error(f"D-ITG process died unexpectedly on {host}")
                        self.error_event.set()
                        return
            ...
            self.logger.error(f"Error in D-ITG thread: {e}")
            self.error_event.set()
        finally:
            # Release ports
            for pair_config in config:
                if 'ditg_port' in pair_config:
                    self.port_manager.release_ditg_port(pair_config['ditg_port'])
