...
class ArpHandler(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.discovery = lookup_service_brick('TopologyManagerV2')
        self.monitor = lookup_service_brick('NetworkMonitor')
    ...
    def _packet_in_handler(self, ev):
        ...
        #LOG.info("shortest---> _packet_in_handler: PacketIn")
        msg = ev.msg
        ...
        # LOG.info(f'shortest--->_packet_in_handler: header_list: {header_list} ')
        if isinstance(arp_pkt, arp.arp):
            ...
            if self.arp_handler(header_list, datapath, in_port, msg.buffer_id):
                ...
                # print(f'arp_dst_ip , {arp_dst_ip}')
                # print(f'shortest--->zzzzzz----> location, {location}')
                if location:  # 如果有这个主机的位置
                    ...
                    #datapath = self.monitor.datapaths_table.get(dpid_dst) # alternitave.
                    
                    # Ensure that datapath_dst is a valid Datapath object
                    if isinstance(datapath, dict):
                        # LOG.info("Retrieved datapath is a dict instead of Datapath object.")
                        return
                    elif datapath is not None:
                        # LOG.info(f'OK?! {datapath}')
                        out = self._build_packet_out(datapath, ofproto.OFP_NO_BUFFER, ofproto.OFPP_CONTROLLER,
                                                     out_port, msg.data)
                        datapath.send_msg(out)
                    else:
                        LOG.info("Datapath not found for dpid: %s", dpid_dst)
                        ###
                    ...
                    for dpid in self.discovery.switch_all_ports_table:
                        for port in self.discovery.switch_all_ports_table[dpid]:
                            if (dpid, port) not in self.discovery.access_table.keys():  # 如果不知道
                                # LOG.info(f'dpid {dpid} to query from datapaths_table')
                                # LOG.info(f'datapath table {self.monitor.datapaths_table[dpid]}')
                                datapath = self.monitor.datapaths_table[dpid]
                                #datapath = self.monitor.datapaths_table.get(dpid)
                                if isinstance(datapath, dict):
                                    # LOG.info("Retrieved datapath is a dict instead of Datapath object.")
                                    return
                                elif datapath is not None:
    ...
    def _cleanup(self):
        """Clear ARP data and stop any running threads/hubs."""
        self.arp_table.clear()
        self.sw.clear()
        self.mac_to_port.clear()
        # If any hub or thread is running, kill it here
        # if hasattr(self, '_my_arp_thread'):
        #     hub.kill(self._my_arp_thread)
